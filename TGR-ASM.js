...
function deHex(hex) {
 return (parseInt(hex, 10).toString()).padStart(8, '0');
}
...
function DemoROM_load() {
 var DemoROM = document.getElementById("asm-data");
 log(DemoROM.selectedIndex,"\n");
 if   (DemoROM.selectedIndex ==  0) {
  
  
 }
 
 ChangeTo.selectedIndex == 0
}

function Display_Bytes(bytes) {
 var table = document.getElementById("output");
 var index=(table.rows.length*16)-16;
 while(index<bytes.length) {
  console.log(index);
  let row = table.insertRow();
  row.insertCell().innerHTML = "0x"+hex(0xFFFFFF0&index,7);
  let z = index;
  for(i=0;i<16;i++) { row.insertCell().id = hex(index,7); index++; }
  row.insertCell().id = hex(z>>4,6)+"_ASCII";
 } index=0;
 while(index<bytes.length) {
  let Ascii = document.getElementById(hex(index>>4,6)+"_ASCII");
  Ascii.innerHTML = "";
  for(x=0;x<16;x++) {
   let Byte = document.getElementById(hex(index,7));
   Byte.innerHTML = (bytes[index])?hex(bytes[index]):"--";
   Ascii.innerHTML += String.fromCharCode(bytes[index]);
   index++;
  }
 }
}

//var bytes = [7,4,56,8,9,4,5,85,7,56,4,5,74,69,6,4,6,74,56,45,9,74,6,4];
//Display_Bytes(bytes);

function reg(x) {
 if (x > 7) { alert("Error Detected! You must give a Register index thats less 8"); return -1; }
 if (type(x) == "number") { return "abcdefgh"[x]; } else { alert("Error Detected! You must give a \"number\" got \""+typeof(x)+"\"..."); return -1; }
}

function combinetochar(a,b) {
 if (b < 0xf && a < 0xf) { return b | a << 4; }
 else { alert("values too large to fit in char"); }
}

function include(File,loc):
 global asm
 try:
  with open(FN,"r") as file:
   asm=asm[:loc]+file.readlines()+asm[loc:]
  print(FN,"loaded...")
 except IOError:
...
function check(number) {
 if (String(number)[:2] == "0x") {
  return deHex(number[2:])
 } else {
  if (hex(number) == "number") {
   return Number(number)
  }
  else { alert("Invalid Value given: \""+str(number)+"\"... if this is hex, please provide 0x at the start of the value"); }
 }
 return -1
}

ASM = {
 file: load(document.getElementById("asm-file")),
...
if len(sys.argv) < 2:
 ...
 address = 0
 preaddr = 0
 instructions = ["raw", "set", "get", "mov", "lmov", "add", "inc", "sub", "dec", "mul", "div", "and", "or", "xor", "bsl", "bsr", "not", "split", "combine", "jmp", "led", "cmpeq", "cmplt", "cmpgt", "rpos", "wram", "rram", "wvram", "rvram", "rsav", "wsav", "rrom", "hlt", "disp", "flags", "dsend", "drecv", "icout", "exec", "rbios", "push", "pop", "call", "ret", "swap", "gclk", "wait", "nop"]
...
#     print("ERROR at line "+str(i+1)+": was expecting REG or Value for IMM but is invalid (got \""+args[1]+"\")"); exit(-1)
   ...
   if line[0] == "push":
    ...
    else:                out+=[0x26,0x01,combinetochar(0,(check(args[0])&0xF000000)>>24),(check(args[0])&0x0FF0000)>>16,(check(args[0])&0x000FF00)>>8,check(args[0])&0x00000FF]
...
#  print("]")
 ...
 reset: function() {//Reset Assembler
  ASM.data = ASM.file[1];
  ASM.name = "";
  ASM.output = [];
 },
 
 save: function() {//Save Output
  saveByteArray(ASM.output,(ASM.name?ASM.name:"rom")+".tgr");
 },
}

window.setInterval(function(){Display_Bytes(ASM.output);});
